...
def CreateOverlay():
    ...
    def ManualCenterConfiguratorX(ValueX):
        ...
        overlay.geometry(
            f"120x120+{config.center_x - config.radius}+{config.center_y - config.radius}"
        )
    ...
    def CreateSlider(root, LabelText, fromV, toV, resolution, command, setValue):
        tk.Label(root, text=LabelText, fg="#9b59b6", bg="black").pack()
        Slider = tk.Scale(
            root,
            from_=fromV,
            to=toV,
    ...
    CreateSlider(
        root,
        "Offset CenterX Manually",
        -100,
        100,
        1,
        ManualCenterConfiguratorX,
        0,
    )
    CreateSlider(
        root,
        "Offset CenterY Manually",
        -100,
        100,
        1,
        ManualCenterConfiguratorY,
        0,
    )
    CreateSlider(
        root,
    ...
    canvas = tk.Canvas(
        overlay, width=120, height=120, bg="blue", bd=0, highlightthickness=0
    )
    canvas.pack()
    config.fovC = canvas.create_oval(
        0, 0, config.radius * 2, config.radius * 2, outline="#9b59b6"  # purple circle
    )
...
def main():
    ...
    while config.Running:
        ...
        results = model.predict(
            source=GameFrame, conf=0.45, classes=[0], verbose=False, max_det=10
        )
        ...
        if (
            abs(moveX) <= displacementX // 2
            and abs(moveY) <= displacementY // 2
            and noDetectionIteration <= 3
        ):
            noDetectionIteration += 1
